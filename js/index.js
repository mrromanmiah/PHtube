...
//             tabs.forEach((tab) => {
//                 tab.classList.remove("bg-[#25252526]");
//             });
//             tab.classList.add("bg-[#FF1F3D]");
...
const handleCategory = async () => {
    ...
    data.data?.forEach((category, index) => {
        const div = document.createElement("div");
        div.innerHTML = `
        <a onclick="handleVideos('${category.category_id}')" class="tab bg-[#25252526] font-medium rounded ${index === 0 ? 'active' : ''}">${category.category}</a>`;
        const tab = div.querySelector("a");
        tab.addEventListener("click", () => {
            const tabs = document.querySelectorAll(".tab");
            tabs.forEach((tab) => {
